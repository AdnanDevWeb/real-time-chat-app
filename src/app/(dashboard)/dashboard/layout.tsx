...
const Layout = async ({children}: layoutProps) => {
    ...
    const unseenRequestCount = (await fetchRedis('smembers', `user: ${session.user.id}: incoming_friend_requests`) as []).length

  return <div className='w-full flex h-screen'>
    <div className='flex h-full w-full max-w-xs grow flex-col gap-y-5 overflow-y-auto border-r border-gray-200 bg-white px-6'>
        <Link href={'/dashboard'} className='flex h-16 shrink-0 items-center'>
            <Icons.Logo className='h-7 w-auto text-indigo-500' />
        </Link>
        ...
        <nav className='flex flex-1 flex-col'>
            <ul role='list' className='flex flex-1 flex-col gap-y-7'>
                        ...
                        { sidebarOptions.map((option) => {
                            ...
                            return (
                                <li key={option.id}>
                                    ...
                                    className='text-gray-7000 hover:text-indigo-600 hover:bg-gray-50 group flex gap-3 rounded-md text-sm p-2 leading-6 font-semibold'
                                    >
                ...
                <li className='-mx-6 mt-auto flex items-center'>
                    <div className='flex lfex-1 items-center gap-x-4 px-6 py-3 text-sm font-semibold leading-6 text-gray-900'>
